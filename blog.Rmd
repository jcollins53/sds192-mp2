...
contributionsByRegion <- house_elections %>%
  left_join(votesPerRegion, by = c("state", "district"))%>%
  left_join(netContributionsByCandidate, by="fec_id") %>%
  mutate(percentVotes = general_votes/totalVotes, incumbent = factor(incumbent), ge_winner = factor(ge_winner)) %>%
  filter( ge_winner != "" & general_votes!= 0)

ggplot(contributionsByRegion, aes(x = netContribution, y = percentVotes, col=ge_winner, fill = ge_winner)) +
